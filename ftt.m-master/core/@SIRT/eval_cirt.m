function [r,f] = eval_cirt(obj, x, z)
...
if obj.int_dir > 0
    ...
    for k = 1:(dx-1)
        ...
        T2  = SIRT.eval_oned_core_213(obj.oneds{k}, obj.cores{k}, x(k,:));
    ...
    % evaluate the updated basis function
    % for the marginal
    T2  = SIRT.eval_oned_core_213(obj.oneds{k}, obj.ys{k}, x(k,:));
    jj  = reshape(reshape(1:rkm*nx, rkm, nx)', [], 1);
    ii  = repmat((1:nx)', 1, rkm);
    B   = sparse(ii(:), jj(:), frl(:), nx, rkm*nx);
    frl_m   = B*T2;
    ...
    for j = 1:dr
        ...
        %
        % evaluate the updated basis function
        T2  = SIRT.eval_oned_core_213(obj.oneds{k}, obj.cores{k}, r(j,:));
    ...
    for j = dx:-1:2
        ...
        % evaluate the updated basis function
        T2  = SIRT.eval_oned_core_231(obj.oneds{k}, obj.cores{k}, x(j,:));
    ...
    % for marginal
    T2  = SIRT.eval_oned_core_231(obj.oneds{k}, obj.ys{k}, x(1,:));
    ii  = reshape(1:rk*nx, [], 1);
    jj  = reshape(repmat(1:nx, rk, 1), [], 1);
    B   = sparse(ii, jj, frg(:), rk*nx, nx);
    frg_m   = T2'*B;
    ...
    % for frg
    T2  = SIRT.eval_oned_core_231(obj.oneds{k}, obj.cores{k}, x(1,:));
    ii  = reshape(1:rk*nx, [], 1);
    jj  = reshape(repmat(1:nx, rk, 1), [], 1);
    B   = sparse(ii, jj, frg(:), rk*nx, nx);
    frg = T2'*B;
